...
class Seasons {
  ...
  next() {
    ...
    return seasonsArray[this.season]
  }
}
...
class Car {
  ...
  refuel(gallons) {
    // ✨ implement
    if(this.miles === this.tank * this.mpg){
    ...
    if (gallons > this.tank){
      this.miles = this.tank * this.mpg
    } else {
      this.miles = gallons * this.mpg
    }
